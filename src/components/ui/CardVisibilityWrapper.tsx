...
export function CardVisibilityWrapper({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <div className={`hidden md:flex fixed top-8 z-20 flex-col gap-3 pointer-events-none transition-all duration-700 ease-in-out ${!isVisible ? 'left-1/2 -translate-x-1/2 items-center' : 'left-8 items-start'
            ...
            >
              {comboCount > 1 && (
                <div className={`relative font-black text-xl italic tracking-wider scale-110 flex items-center transition-all duration-700 ${!isVisible ? 'justify-center origin-center ml-0' : 'justify-start origin-left ml-2'}`}>
                  ...
                  {/* MLG Aktív réteg */}
                  <div className={`absolute inset-0 flex items-center whitespace-nowrap transition-opacity duration-500 ease-in-out ${!isVisible ? 'justify-center' : 'justify-start'} ${comboState === 'active' ? 'opacity-100 animate-mlg-colors' : 'opacity-0'
                    }`}>
                    {comboCount}x COMBO!
                  </div>

                  {/* Pulzáló elhalványuló réteg */}
                  <div className={`absolute inset-0 flex items-center whitespace-nowrap transition-opacity duration-500 ease-in-out ${!isVisible ? 'justify-center' : 'justify-start'} ${comboState === 'fading' ? 'opacity-100 animate-pulse text-[#f59e0b] drop-shadow-[0_0_8px_rgba(245,158,11,0.8)]' : 'opacity-0'
                    }`}>
                    {comboCount}x COMBO!
                  </div>
                </div>
