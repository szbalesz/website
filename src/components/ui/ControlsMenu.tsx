...
export function ControlsMenu() {
  ...
  return (
    <TooltipProvider delay={0}>
      <div className="fixed bottom-6 right-6 z-[60] hidden md:flex items-center justify-center p-1 rounded-full bg-black/40 hover:bg-black/60 border border-white/10 transition-all backdrop-blur-sm shadow-lg overflow-hidden group animate-button-in" style={{ animationDelay: '2.4s' }}>
        ...
        <div className="flex items-center gap-1 max-w-0 opacity-0 overflow-hidden group-hover:max-w-[300px] group-hover:opacity-100 group-hover:ml-1 group-hover:mr-1 transition-all duration-500 ease-in-out">
          ...
          <Tooltip>
            <TooltipTrigger
              onClick={() => setEmotesEnabled(!emotesEnabled)}
              className={`flex items-center justify-center w-9 h-9 rounded-full transition-all cursor-pointer ${emotesEnabled ? 'hover:bg-white/20 text-white/70 hover:text-white' : 'bg-red-500/20 hover:bg-red-500/40 text-red-400'}`}
            >
              <Ghost className={`w-5 h-5 transition-opacity ${emotesEnabled ? "opacity-100" : "opacity-50"}`} />
            </TooltipTrigger>
            <TooltipContent side="top" className="bg-black/80 text-white border-white/10 backdrop-blur-md">
              {emotesEnabled ? "Emote-ok elrejtése" : "Emote-ok megjelenítése"}
            </TooltipContent>
          </Tooltip>
